...
func RRSchedule(w io.Writer, title string, processes []Process) {
	var (
		...
		schedule        = make([][]string, len(processes))
		gantt           = make([]TimeSlice, 0)
		currentTime     float64 // Change serviceTime to float64
	...
	for len(remainingBurst) > 0 {
		for pid, burst := range remainingBurst {
			if burst <= quantum {
				...
				start := currentTime
				currentTime += float64(burst) // Convert burst to float64
				completion := currentTime

				// Update metrics
				totalWait += float64(currentTime - float64(processes[pid-1].ArrivalTime) - float64(processes[pid-1].BurstDuration))
				totalTurnaround += float64(currentTime - float64(processes[pid-1].ArrivalTime))

				// Add the process to the Gantt chart
				gantt = append(gantt, TimeSlice{
					PID:   pid,
					Start: int64(start),      // Convert to int64
					Stop:  int64(completion), // Convert to int64
				})
				...
				schedule[pid-1] = []string{
					...
					fmt.Sprint(currentTime - float64(processes[pid-1].ArrivalTime) - float64(processes[pid-1].BurstDuration)),
					fmt.Sprint(currentTime - float64(processes[pid-1].ArrivalTime)),
					fmt.Sprint(completion),
				}
				...
				start := currentTime
				currentTime += float64(quantum) // Convert quantum to float64
				...
				gantt = append(gantt, TimeSlice{
					PID:   pid,
					Start: int64(start),       // Convert to int64
					Stop:  int64(currentTime), // Convert to int64
				})
			}
		}
	}
	...
	aveWait := totalWait / count
	aveTurnaround := totalTurnaround / count
	aveThroughput := float64(len(processes)) / float64(currentTime) // Use float64 currentTime
